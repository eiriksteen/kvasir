...
interface AnalysisItemStructuredTabProps {
  analysisObjectId: UUID;
  projectId: UUID;
}


const AnalysisItemStructuredTab: React.FC<AnalysisItemStructuredTabProps> = ({
  analysisObjectId,
  projectId,
}) => {
  const {
  ...
  return (
    <DndContext onDragEnd={handleDragEnd}>
      <div className="flex h-full">
        ...
        <div className="flex-1 text-sm text-zinc-300 flex flex-col overflow-y-auto">
          {/* Sections Content */}
          ...
          {orderedItems.length === 0 && (
            <div className="flex-1 text-center py-8">
              <Bot size={48} className="mx-auto text-zinc-600 mb-4" />
              <p className="text-zinc-500 mb-4">No sections created yet.</p>
            </div>
          )}
        </div>
      </div>
...
export default AnalysisItemStructuredTab; 