...
from kvasir_agents.agents.v1.extraction.helper_agent import (
    data_source_agent,
    dataset_agent,
    pipeline_agent,
    model_agent
)
from kvasir_agents.agents.v1.shared_tools import read_files_tool
...
from kvasir_ontology.graph.data_model import EdgeCreate
...
class EntityGroupToCreate(BaseModel):
    name: str
    description: str
    x_position: float
    y_position: float
...
async def submit_entities_to_create_or_update(
    ...
    groups_to_create: List[EntityGroupToCreate],
    models_to_create: List[ModelToCreate]
) -> str:
    await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Starting submit_entities_to_create_or_update: {len(entities_to_create)} entity/ies to create, {len(entities_to_update)} to update, {len(groups_to_create)} group(s) to create, {len(models_to_create)} model(s) to create", "tool_call")
    ...
    await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Groups to create: {groups_to_create}", "result")
    ...
    model_name_to_id_map: dict[str, uuid.UUID] = {}

    entities_by_group: dict[str, List[EntityToCreate]] = {}
    for entity in entities_to_create:
        if entity.group_name:
            if entity.group_name not in entities_by_group:
                entities_by_group[entity.group_name] = []
            entities_by_group[entity.group_name].append(entity)

    await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Starting group creation phase: processing {len(groups_to_create)} group(s)", "result")
    for i, group in enumerate(groups_to_create):
        group_entities = entities_by_group.get(group.name, [])
        await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Processing group {i}/{len(groups_to_create)}: {group.name} with {len(group_entities)} entity/ies", "result")
        ...
        if entity_type == "data_source":
            ...
            created_entities = await ctx.deps.ontology.insert_data_source_group(
                name=group.name,
                description=group.description,
                data_sources=data_source_creates,
                x_position=group.x_position,
                y_position=group.y_position,
                edges=[]
            )
            for entity_obj in created_entities:
                name_to_id_map[entity_obj.name] = entity_obj.id
            ...
            created_entities = await ctx.deps.ontology.insert_dataset_group(
                name=group.name,
                description=group.description,
                datasets=dataset_creates,
                x_position=group.x_position,
                y_position=group.y_position,
                edges=[]
            )
            ...
            created_entities = await ctx.deps.ontology.insert_pipeline_group(
                name=group.name,
                description=group.description,
                pipelines=pipeline_creates,
                x_position=group.x_position,
                y_position=group.y_position,
                edges=[]
            )
            ...
            created_entities = await ctx.deps.ontology.insert_analysis_group(
                name=group.name,
                description=group.description,
                analyses=analysis_creates,
                x_position=group.x_position,
                y_position=group.y_position,
                edges=[]
            )
    ...
    ungrouped_entities = [e for e in entities_to_create if not e.group_name]
    await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Starting entity creation phase: processing {len(ungrouped_entities)} entity/ies", "result")
    for i, entity in enumerate(ungrouped_entities, 1):
        await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Creating entity {i}/{len(ungrouped_entities)}: {entity.name} (type: {entity.type})", "result")
    ...
    for i, model_to_create in enumerate(models_to_create):
        await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Creating model {i}/{len(models_to_create)}: {model_to_create.name}", "result")
    ...
    instantiations_by_group: dict[str,
                                  List[tuple[ModelInstantiationToCreate, uuid.UUID]]] = {}
    ungrouped_instantiations: List[tuple[ModelInstantiationToCreate, uuid.UUID]] = [
    ]
    for model_to_create in models_to_create:
        model_id = model_name_to_id_map[model_to_create.name]
        for instantiation in model_to_create.instantiations_to_create:
            if instantiation.group_name:
                if instantiation.group_name not in instantiations_by_group:
                    instantiations_by_group[instantiation.group_name] = []
                instantiations_by_group[instantiation.group_name].append(
                    (instantiation, model_id))
            else:
                ungrouped_instantiations.append((instantiation, model_id))

    for group in groups_to_create:
        if group.name in instantiations_by_group:
            group_instantiations = instantiations_by_group[group.name]
            await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Creating model instantiation group: {group.name} with {len(group_instantiations)} instantiation(s)", "result")
            model_instantiated_creates = [
                ...
                for inst, model_id in group_instantiations
            ]
            created_instantiations = await ctx.deps.ontology.insert_model_instantiated_group(
                name=group.name,
                description=group.description,
                models_instantiated=model_instantiated_creates,
                x_position=group.x_position,
                y_position=group.y_position,
                edges=[]
            )
            for inst_obj in created_instantiations:
                name_to_id_map[inst_obj.name] = inst_obj.id

    for instantiation, model_id in ungrouped_instantiations:
        ...
        await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Created instantiation: {instantiation.name}", "result")
    ...
    for i, entity in enumerate(entities_to_create, 1):
        if entity.name not in name_to_id_map:
            continue
    ...
    if coroutines:
        ...
        current_entity_graph_description = await ctx.deps.ontology.describe_mount_node(include_positions=True)
        ...
        await ctx.deps.callbacks.log(ctx.deps.user_id, ctx.deps.run_id, f"Current entity graph: {current_entity_graph_description}", "result")
...
async def submit_edges_to_create(ctx: RunContext[ExtractionDeps], edges: List[EdgeCreate]) -> str:
