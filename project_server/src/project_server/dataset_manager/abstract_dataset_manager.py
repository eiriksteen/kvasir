...
from project_server.client import project_client
...
class AbstractDatasetManager(ABC):
    ...
    @abstractmethod
    async def upload_dataset(self, dataset: DatasetCreate) -> None:
        """
    ...
    async def _fetch_group_metadata_from_main_server(self, group_id: uuid.UUID) -> ObjectGroupAPI:
        """
        - Fetch group metadata from main server
        """

        response = await self.client.send_request("get", f"/data-objects/object-group/{group_id}", {})
        response_json = await response.json()

        return ObjectGroupAPI(**response_json)

    async def _fetch_dataset_metadata_from_main_server(self, dataset_id: uuid.UUID) -> DatasetWithObjectGroupsAPI:
        """
        - Fetch dataset metadata from main server
        """

        response = await self.client.send_request("get", f"/data-objects/dataset/{dataset_id}", {})
        response_json = await response.json()

        return DatasetWithObjectGroupsAPI(**response_json)
    ...
    async def _upload_dataset_metadata_to_main_server(self, dataset_create: DatasetCreate) -> DatasetWithRawData:
        ...
        response = await self.client.send_request("post", "/data-objects/dataset", {
            "files": files,
            "metadata": json.dumps(asdict(dataset_create_api))
        })
        response_json = await response.json()

        dataset_from_api = DatasetWithObjectGroupsAPI(**response_json)
