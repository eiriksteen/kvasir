...
from project_server.client import ProjectClient, get_project, get_echart
...
@router.post("/get-chart/{chart_id}")
async def get_chart(
    ...
    echart = await get_echart(client, request.chart_id)
    script_path = Path(echart.chart_script_path)
    try:
