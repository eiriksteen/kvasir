...
async def get_last_element_in_section(section_id: uuid.UUID, notebook_id: uuid.UUID | None = None) -> Tuple[Literal['analysis_result', 'notebook_section'], uuid.UUID] | Tuple[None, None]:
    ...
    result = await fetch_one(
        select(analysis_result).where(and_(analysis_result.c.section_id ==
                                           section_id, analysis_result.c.next_id == None))
    )
    if result:
    ...
    result = await fetch_one(
        select(notebook_section).where(and_(notebook_section.c.parent_section_id == section_id,
                                            notebook_section.c.next_id == None, notebook_section.c.notebook_id == notebook_id))
    )
    if result:
...
async def generate_notebook_report(analysis: AnalysisInDB, notebook: Notebook, include_code: bool, user_id: uuid.UUID) -> str:
    ...
    if notebook:
        ...
        first_section = next(
            (s for s in all_sections if s.id not in referenced_ids), None)
        first_result = next(
            (r for r in all_results if r.id not in referenced_ids), None)

        if first_section:
            ordered_items = build_ordered_list(
                all_sections, all_results, first_section.id, "notebook_section")
        elif first_result:
            ordered_items = build_ordered_list(
                all_sections, all_results, first_result.id, "analysis_result")
        else:
            ordered_items = []
...
async def section_to_markdown(section: NotebookSection, include_code: bool, user_id: uuid.UUID, level: int = 1) -> str:
    ...
    first_child_section = next(
        (s for s in child_sections if s.id not in referenced_ids), None)
    first_result = next(
        (r for r in analysis_results if r.id not in referenced_ids), None)

    ordered_children = []
    if first_child_section:
        ordered_children = build_ordered_list(
            child_sections, analysis_results, first_child_section.id, "notebook_section")
    elif first_result:
        ordered_children = build_ordered_list(
            child_sections, analysis_results, first_result.id, "analysis_result")
