...
interface IntegrationJobDetailProps {
  jobId: string;
  jobName: string;
  jobStatus: string;
  onBack: () => void;
...
export default function IntegrationJobDetail({ jobId, jobName, jobStatus, onBack }: IntegrationJobDetailProps) {
  const { messages, submitFeedback, submitApproval, isConnected } = useIntegrationAgent(jobId);
    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-[#0a101c] to-[#050a14]">
            ...
            >
             ...
             <div className={`flex items-center text-xs font-medium px-2.5 py-1 rounded-full border ${
              ...
              {isConnected ? <Wifi size={13} className="mr-1.5" /> : <WifiOff size={13} className="mr-1.5"/>}
               {isConnected ? jobStatus.replace(/_/g, ' ').toUpperCase() : 'CONNECTING...'}
            </div>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-[#1f2937] bg-[#0a101c]/90 backdrop-blur-sm flex-shrink-0">
        <div className="min-h-[40px] flex items-center">
          {jobStatus === 'awaiting_approval' && !isDisapprovalFeedback ? (
            ...
            <div className="flex rounded-lg bg-[#111827]/80 border border-zinc-700/80 overflow-hidden shadow-inner items-center focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-transparent w-full">
              <input
                ...
                disabled={!isConnected || jobStatus === 'completed' || jobStatus === 'failed'}
              />
              <button
                ...
                disabled={!userInput.trim() || !isConnected || jobStatus === 'completed' || jobStatus === 'failed'}
              >
                <Send size={18} />
              </button>
            </div>
