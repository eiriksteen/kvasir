...
def _get_section_description(section: NotebookSection) -> str:
    ...
    if section.section_description:
        ...
        if section.parent_section_id:
            lines.append(f"This section is a subsection of the section with id: {section.parent_section_id}")

    if len(section.analysis_results) > 0:
        lines.append(f"The following analysis results belong to the section with id {section.id}:")
    for i, analysis_result in enumerate(section.analysis_results):
        ...
        lines.append(f"The result was from this code: {analysis_result.python_code}")
        lines.append("]")
