...
import { Calendar, Loader2, Pause, Check, X, Layers, Target, Settings } from 'lucide-react';
...
export default function ModelCompact({ model, gradientClass, onClick }: ModelProps) {
  ...
  return (
        ...
        >
            ...
            <div className="flex items-start gap-3">
                <h4 className="font-mono text-gray-200 text-base" title={model.name}>
                {model.name}
                </h4>
            ...
            <p className="text-sm text-zinc-400 mt-2 line-clamp-2 flex-grow" title={model.description}>
                {model.description}
            </p>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <span>{model.modality.name}</span>
            </div>
            <div className="flex items-center gap-2">
                <Target size={14} className="flex-shrink-0" />
                <span>{formatNumber(model.tasks.length)} task(s)</span>
            </div>
            <div className="flex items-center gap-2">
                <Settings size={14} className="flex-shrink-0" />
                <span>{formatNumber(model.configParameters.length)} config param(s)</span>
            </div>
            <div className="flex items-center gap-2">
