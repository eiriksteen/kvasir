...
from synesis_api.agents.model_integration_alt.setup_agent.agent import (
    SetupAgentOutput,
    SetupAgentOutputWithScript,
    setup_agent
)
from synesis_api.agents.model_integration_alt.implementation_agent.agent import (
    ModelAnalysisOutput,
...
from synesis_api.agents.model_integration_alt.deps import ModelIntegrationDeps
...
from synesis_api.agents.model_integration_alt.input_structures import get_input_structure, get_config_definition_code
from synesis_api.agents.model_integration_alt.output_structures import get_output_structure
...
class ModelIntegrationRunner:
    ...
    async def _run_training_script(self, training_script: str) -> None:
        ...
        if err:
            await self._log_message(f"Error running training script: {err}", "result")
            raise RuntimeError(f"Error running training script: {err}")

        await self._log_message("Training script run successfully", "result")
        return out
    ...
    async def _run_setup(self) -> SetupAgentOutputWithScript:
        ...
        if cached_setup:
            await self._log_message("Found cached setup output, using cached result", "result")
    ...
    async def _run_model_analysis(self, python_version: str) -> Tuple[ModelAnalysisOutput, list[ModelMessage]]:
        ...
        if cached_analysis:
            await self._log_message("Found cached model analysis output, using cached result", "result")
        ...
        await self._log_message("Model analysis completed successfully", "result")
    ...
    async def _run_implementation(
        ...
        for task in model_analysis_output.supported_tasks:
            ...
            if cached_planning:
                await self._log_message(f"Found cached planning output for task: {task}, using cached result", "result")
                planning_output = ImplementationPlanningOutput(
                    **cached_planning["output"])
            else:
                self._set_stage("implementation_planning", task)
                await self._log_message(f"Starting implementation for task: {task}")
            ...
            if cached_training:
                await self._log_message(f"Found cached training output for task: {task}, using cached result", "result")
            ...
            await self._log_message(f"Training script successfully implemented for task: {task}", "result")
            ...
            if cached_inference:
                await self._log_message(f"Found cached inference output for task: {task}, using cached result", "result")
            ...
            await self._log_message(f"Inference script successfully implemented for task: {task}", "result")
    ...
    async def _save_results(self,
        ...
        for i, task in enumerate(model_analysis_output.supported_tasks):
            ...
            with open(str(inference_save_dir / f"{task}.py"), "w") as f:
                f.write(inference_outputs[i].script)
            with open(str(training_save_dir / f"{task}.py"), "w") as f:
                f.write(training_outputs[i].script)
        ...
        with open(str(model_save_dir / "config.py"), "w") as f:
            f.write(model_analysis_output.config_code)
        ...
        await insert_model(
            ...
            setup_script_path=str(model_save_dir / "setup.sh"),
