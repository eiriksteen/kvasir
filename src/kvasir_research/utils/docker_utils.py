...
from kvasir_research.secrets import (
    SANDBOX_DIR,
...
def _create_new_project_container_sync(project_id: UUID, package_name: str, image_name: str = "research-sandbox") -> docker.models.containers.Container:
    """Synchronous function to create a new project container."""
    docker_client = docker.from_env()
    container_name = str(project_id)

    project_dir = SANDBOX_HOST_DIR / "exp" / str(project_id)
    project_package_dir = project_dir / package_name

    if not project_dir.exists():
        _create_empty_project_package(project_id, package_name)
    ...
    container = docker_client.containers.create(
        ...
        tty=True,
        stdin_open=True,
        working_dir=f"/app/{package_name}",
        volumes={str(project_package_dir): {
            "bind": f"/app/{package_name}", "mode": "rw"}}
    )
    container.start()
    return container


async def create_project_container_if_not_exists(project_id: UUID, package_name: str, image_name: str = "research-sandbox") -> Path:
    docker_client = docker.from_env()
    container_name = str(project_id)
    ...
    try:
        ...
        await asyncio.to_thread(_create_new_project_container_sync, project_id, package_name, image_name)

    # Returns the project path
    return Path(f"/app/{package_name}")


async def install_package_after_start(container_name: str, package_name: str) -> None:
    _, err = await run_shell_code_in_container(
        "pip install -e .",
        container_name,
        cwd=f"/app/{package_name}"
    )
...
def _create_empty_project_package(project_id: UUID, package_name: str) -> None:
    """Create directory structure, pyproject.toml, and Dockerfile for first-time project setup"""
    project_dir = SANDBOX_DIR / "exp" / str(project_id)
    project_package_dir = project_dir / package_name
...
def delete_project_container_if_exists(project_id: UUID) -> None:
    docker_client = docker.from_env()
    container_name = str(project_id)
...
async def ensure_directory_exists(directory_path: str, container_name: str):
    cmd = [
        "docker", "exec", "-i",
        container_name,
