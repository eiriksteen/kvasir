import { fetchDatasets, fetchJobs, streamChat, fetchMessages, createConversation, updateContext, fetchAnalysis } from "@/lib/api";
...
export const useAnalysis = (edaJobId: string) => {
  const {data: session} = useSession();
  ...
  const { data, error, isLoading } = useSWR(`${URL}/eda/eda-job-results/${edaJobId}`, () => fetchAnalysis(session.APIToken.accessToken, edaJobId));
...
export const useRefreshAnalysis = (runningJobs: Job[]) => {
  const {mutate} = useSWRConfig();
  
  useEffect(() => {
    if (runningJobs.length === 0) {
      mutate(`${URL}/eda/analysis`);
    }
  }, [runningJobs]);
};
