...
class Datasets(DatasetInterface):
    ...
    async def get_object_group_submission_code(self) -> Tuple[str, str]:
        ...
        submission_code = (
            ...
            f"    \n"
            f"    # Extract metadata (everything except dataset_id which goes in URL)\n"
            f"    metadata = {{k: v for k, v in og_dict.items() if k != 'dataset_id'}}\n"
            f"    \n"
            f"    # Note: The endpoint requires files, but for sandbox submission we'll submit metadata only\n"
            f"    # The actual file handling may need to be done separately\n"
            ...
            f"    }}\n"
            f"    \n"
            f"    # Submit metadata as JSON (files would need to be handled separately)\n"
            f"    response = requests.post(url, headers=headers, json=metadata)\n"
        ...
        submission_description = (
            "The submission code expects a variable named 'object_groups_dict' that contains either:\n"
            "- A single dictionary following the ObjectGroupCreate schema\n"
            "- A list of dictionaries, each following the ObjectGroupCreate schema\n"
            "\n"
            ...
            "- 'modality_fields': Dictionary with modality-specific fields\n"
            "- 'objects_files': List of ObjectsFile dictionaries with file metadata\n"
            "- Additional fields as needed for the ObjectGroupCreate schema\n"
            "\n"
            "The code will POST each object group to the API endpoint and print the response(s) as JSON.\n"
            "Note: The endpoint typically requires files, but for sandbox submission the metadata is submitted.\n"
        )
