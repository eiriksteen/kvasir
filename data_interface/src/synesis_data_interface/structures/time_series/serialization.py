...
def serialize_time_series_dataclass_to_api_payload(data_structure: TimeSeriesStructure) -> List[TimeSeries]:
...
def serialize_dataframes_to_parquet(data_structure: TimeSeriesStructure) -> Dict[str, bytes]:
    """
    ...
    Args:
        data_structure: A TimeSeriesStructure instance
...
) -> TimeSeriesStructure | MetadataStructure:
    """
    Deserialize parquet data back to a TimeSeriesStructure.

    This is the reverse operation of serialize_dataframes_to_parquet.
    Takes parquet bytes from HTTP submission and converts them back to a TimeSeriesStructure.

    Args:
        parquet_data: Dictionary mapping second_level_id to parquet bytes (from HTTP)
        only_metadata: If True, only return MetadataStructure without time series data

    Returns:
        A TimeSeriesStructure or MetadataStructure instance
    """
