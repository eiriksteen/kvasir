    ...
    return (
        <div className="w-full">
            ...
            <div 
                ...
                style={style}
                {...attributes}
                className={`bg-[#0a101c] rounded-lg py-2 px-4 mb-2 transition-opacity w-full min-w-0 ${
                    isDragging ? 'opacity-50' : 'opacity-100'
                }`}
            >   
                {isDragging ? (
                    <div className="w-full h-16 bg-purple-900/30 flex items-center justify-center rounded-lg">
                        <div className="flex items-center gap-2">
                            <FileSearch size={16} className="text-purple-400 animate-pulse" />
                            <span className="text-sm text-purple-400">Moving analysis result...</span>
                        </div>
                    </div>
                ) : (
                    <>
                                    ...
                                    {showEditAnalysis ? (
                                        <div className="space-y-3">
                                            <textarea
                                                ...
                                                className="w-full px-3 py-2 text-xs rounded border border-gray-600 bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-gray-500 resize-none"
                        ...
                        {showDetails && (
                            <div className="mt-4 pt-4 border-t border-[#271a30]">
                                {/* Datasets Section */}
                                {(currentAnalysisResult.datasetIds.length > 0 || currentAnalysisResult.dataSourceIds?.length > 0) && (
                                    <div className="mb-3">
                                        ...
                                        {showDatasets && (
                                            <div className="mt-2 bg-[#1a1625] rounded border border-[#271a30] p-3">
                                                <div className="space-y-2">
                                                    {currentAnalysisResult.datasetIds.map((datasetId: string, index: number) => (
                                                        <div key={datasetId} className="flex items-center gap-2 text-xs text-zinc-400">
                                                    ...
                                                    {currentAnalysisResult.dataSourceIds?.map((dataSourceId: string, index: number) => (
                                                        <div key={dataSourceId} className="flex items-center gap-2 text-xs text-zinc-400">
                                ...
                                {currentAnalysisResult.pythonCode && (
                                    <div className="border-t border-[#271a30] pt-3">
                                        <button
                        ...
                        {plots && plots.map((plot: BasePlot) => (
                            <div className="mt-4 pt-4 border-t border-[#271a30]" key={plot.id}>
                                <div className="mb-3">
                                    <div className="h-96 bg-[#1a1625] rounded border border-[#271a30] p-3">
                                        <EChartWrapper
                                            plot={plot}
                        ...
                        {tables && tables.map((table: BaseTable) => (
                            <div className="mt-4 pt-4 border-t border-[#271a30]" key={table.id}>
                                <TablesItem
                                    table={table}
                                    aggregationData={analysisResultData[analysisResult.id] as AggregationObjectWithRawData}
                                    analysisResultId={analysisResult.id}
