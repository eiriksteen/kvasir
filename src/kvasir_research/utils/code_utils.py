...
async def run_python_code_in_container(
        ...
        max_output_length: int = 20000) -> Tuple[str, str]:
    ...
    if truncate_output and len(out_str) > max_output_length:
        truncation_msg = f"\n\n... [Output truncated: showing {max_output_length:,} of {len(out_str):,} characters]"
        out_str = out_str[:max_output_length] + truncation_msg
