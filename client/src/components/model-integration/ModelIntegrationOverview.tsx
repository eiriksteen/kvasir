...
export default function ModelIntegrationOverview({ setCurrentView }: ModelIntegrationOverviewProps) {
  ...
  const { jobs } = useJobs('model_integration'); 
  ...
  if (selectedJobId) {
    return (
      <ModelIntegrationJobDetail 
        integrationName={jobs.find(job => job.id === selectedJobId)?.jobName || ''}
        integrationStatus={jobs.find(job => job.id === selectedJobId)?.status || ''}
        onBack={handleBack}
        jobId={selectedJobId}
      />
    );
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border-b border-[#101827] flex-shrink-0">
        <h3 className="text-md font-semibold text-zinc-200">Integration Jobs Overview</h3>
      </div>
      <div className="flex-grow p-4 overflow-y-auto space-y-3">
        {jobs.length === 0 ? (
          <div className="text-center text-zinc-500 pt-16">
             <FilePlus size={32} className="mx-auto mb-3 opacity-50"/>
             <p className="font-medium text-zinc-400">No model integration jobs found.</p>
