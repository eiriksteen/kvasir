...
from kvasir_api.auth.service import get_current_user, oauth2_scheme
from kvasir_api.auth.schema import User
from kvasir_api.modules.ontology.service import create_ontology_for_user

from kvasir_ontology.graph.data_model import EntityGraph, CHILD_TYPE_LITERAL, EdgeCreate, EdgeBase
...
from kvasir_agents.agents.v1.extraction.agent import run_extraction_agent
...
@router.post("/{mount_node_id}/data-source", response_model=DataSource)
async def insert_data_source_endpoint(
    mount_node_id: UUID,
    request: DataSourceInsertRequest,
    user: Annotated[User, Depends(get_current_user)] = None
) -> DataSource:
    ontology = create_ontology_for_user(user.id, mount_node_id)
    return await ontology.insert_data_source(request.data_source, request.edges)


@router.post("/{mount_node_id}/dataset", response_model=Dataset)
...
@router.post("/{mount_node_id}/analysis", response_model=Analysis)
...
@router.post("/{mount_node_id}/pipeline", response_model=Pipeline)
...
@router.post("/{mount_node_id}/model-instantiated", response_model=ModelInstantiated)
...
@router.post("/{mount_node_id}/files-data-sources", response_model=List[DataSource])
async def insert_files_data_sources_endpoint(
    ...
    if file_paths:
        group_info = await ontology.graph.get_node(mount_node_id)
...
@router.delete("/{mount_node_id}/data-source/{data_source_id}")
...
@router.delete("/{mount_node_id}/dataset/{dataset_id}")
...
@router.delete("/{mount_node_id}/model-instantiated/{model_instantiated_id}")
...
@router.get("/{mount_node_id}/data-sources", response_model=List[DataSource])
...
@router.get("/{mount_node_id}/datasets", response_model=List[Dataset])
...
@router.get("/{mount_node_id}/pipelines", response_model=List[Pipeline])
...
@router.get("/{mount_node_id}/models-instantiated", response_model=List[ModelInstantiated])
...
@router.get("/{mount_node_id}/analyses", response_model=List[Analysis])
...
@router.delete("/{mount_node_id}/pipeline/{pipeline_id}")
...
@router.delete("/{mount_node_id}/analysis/{analysis_id}")
...
@router.delete("/{mount_node_id}/entity-branch/{node_id}")
async def delete_entity_branch_endpoint(
    mount_node_id: UUID,
    node_id: UUID,
    user: Annotated[User, Depends(get_current_user)] = None
) -> str:
    ontology = create_ontology_for_user(user.id, mount_node_id)
    await ontology.delete_entity_branch(node_id)
    return f"Entity branch deleted successfully: {node_id}"


@router.get("/{mount_node_id}/entity-graph", response_model=EntityGraph)
...
@router.get("/{mount_node_id}/entities", response_model=List[Union[DataSource, Dataset, Pipeline, ModelInstantiated, Analysis]])
...
@router.get("/{mount_node_id}/describe-entity-graph")
async def describe_entity_graph(
    mount_node_id: UUID,
    user: Annotated[User, Depends(get_current_user)] = None
) -> str:
    ontology = create_ontology_for_user(user.id, mount_node_id)
    description = await ontology.describe_mount_node()
    return description


@router.get("/{mount_node_id}/describe-entity/entities/{entity_id}")
async def describe_entity(
    mount_node_id: UUID,
    entity_id: UUID,
    entity_type: CHILD_TYPE_LITERAL = Query(..., description="Entity type"),
...
@router.get("/{mount_node_id}/run-extraction")
async def run_extraction(
    ...
    ontology = create_ontology_for_user(
        user.id, mount_node_id, bearer_token=token)
    node_object = await ontology.graph.get_node(mount_node_id)
